...
async function resolveRound(){
  st.busy=true;render();await initiative();
  ...
  draw(st.p);draw(st.b);log(`🔄 เริ่มรอบที่ ${st.round} — ทั้งสองฝ่ายจั่วการ์ด 1 ใบ`);st.busy=false;render()
}
async function selectPlayerCard(i){
  ...
  const bi=choose();st.b.pending=st.b.hand.splice(bi,1)[0];log(`🤖 บอทเลือกการ์ดแล้ว`);render();await wait(500);resolveRound()
}
function showGuide(){
...
function newGame(){
  deck=buildDeck();st={p:player('ผู้เล่น'),b:player('บอท'),round:1,busy:false,first:Math.random()<.5?'p':'b'};
  draw(st.p,5);draw(st.b,5);$('start').classList.add('hidden');$('end').classList.add('hidden');$('game').classList.remove('hidden');$('log').innerHTML='';$('last').textContent='การ์ดที่ใช้จะแสดงตรงนี้';log('🎮 เริ่มเกม — ทั้งสองฝ่ายจะเลือกการ์ดก่อน แล้วจึงประมวลผลพร้อมกันตามลำดับที่สุ่ม');log('🃏 จำกัดเด็คสูงสุด 60 ใบ');render()
}
